...
class FFmpegDownloader {
  ...
  static saveVersion() {
    try {
      const version = execSync(`"${path.join(this.getBinPath(), 'ffmpeg')}" -version 2>&1`, {
        timeout: 5000,
        encoding: 'utf-8'
      });
  ...
  static async install() {
    try {
      ...
      const archivePath = await this.downloadFFmpeg((progress) => {
        console.log(`[FFmpegDownloader] Download progress: ${progress.percentage}% (${(progress.downloaded / 1024 / 1024).toFixed(1)}MB / ${(progress.total / 1024 / 1024).toFixed(1)}MB @ ${progress.speed.toFixed(1)}MB/s)`);
      });
