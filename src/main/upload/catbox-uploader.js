...
class CatboxUploader {
  ...
  async upload(filePath, onProgress = () => {}) {
    ...
    console.log("[UPLOADER] Size:", (fileSize / 1024 / 1024).toFixed(2), "MB");

    if (fileSize > this.catboxLimit) {
      console.log("[UPLOADER] Using Litterbox (file > 200MB)");
      return await this.uploadWithRetry(filePath, onProgress, "litterbox");
    }

    console.log("[UPLOADER] Using Catbox.moe");
  ...
  async uploadWithRetry(filePath, onProgress, service) {
    ...
    for (let attempt = 1; attempt <= this.maxRetries; attempt++) {
      try {
        console.log(`[UPLOADER] Attempt ${attempt}/${this.maxRetries}`);
        ...
        lastError = error;
        console.error(`[UPLOADER] Attempt ${attempt} failed:`, error.message);

        if (attempt < this.maxRetries) {
          const delay = attempt * 2000;
          console.log(`[UPLOADER] Waiting ${delay}ms before trying again...`);
      ...
      const req = https.request(options, (res) => {
        ...
        res.on("end", () => {
          if (res.statusCode === 200 && data.startsWith("https://")) {
            ...
            reject(new Error(`Error: ${data || res.statusCode}`));
          }
        });
      });

      req.on("error", (error) => {
        console.error("[CATBOX] Error:", error.code);
        reject(error);
      });
  ...
  async uploadToLitterbox(filePath, onProgress) {
    return new Promise((resolve, reject) => {
      ...
      req.on("error", (error) => {
        console.error("[LITTERBOX] Error:", error.code);
        reject(error);
      });
